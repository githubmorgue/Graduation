...
def solve_mp_model(r, params, dual_vars):
    ...
    if model.solCount > 0:
        ...
        for sol_idx in range(num_solutions):
            ...
            for t in range(1, params.T + 1):
                for b in range(1, params.B + 1):
                    # 注意：使用 .Xn 获取特定 SolutionNumber 的值
                    current_sol_dict[(t, b)] = x[t, b].Xn

            # 存储 A 的值
            A_val = A.Xn

            # 创建 SolutionInfo 对象（假设这是你定义的类）
            # 这里需要根据你的 SolutionInfo 实际构造函数参数进行调整
            sol_info = SolutionInfo(
                x_vars=current_sol_dict,
                A=A_val,
                obj_value=obj_val
            )

            solutions_with_obj.append((obj_val, sol_info))

        # 根据题目要求：按目标值降序排列（最后一个元素是最优解）
        # solutions_with_obj.sort(key=lambda x: x[0], reverse=True)

        # 提取排序后的 SolutionInfo 对象列表
        all_solutions = [sol for obj, sol in solutions_with_obj]

        # --- 调试输出 ---
        # 如果你需要打印输出，可以取消下面的注释
        # for idx, (obj, sol) in enumerate(solutions_with_obj):
        #     print(f"\n解 {idx + 1} (目标值: {obj}):")
        #     for (t, b), val in sol.x_vars.items():
        #         if val > 0.5: # 二元变量，打印非零的
        #             print(f"  x[{t}][{b}] = {val}")
